...
export function SentPage() {
  ...
  const { data, isLoading, isError, error: queryError, refetch, isFetching } = useQuery({
    ...
    queryFn: () => emailsApi.getEmails({ folder: 'sent' }),
  });
      ...
      >
        <div className="px-4 py-3 border-b border-slate-800 bg-slate-900/90 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-white">Sent Messages</h2>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-slate-850/80">
          {isLoading ? (
            ...
            <ErrorState
              message={(queryError as any)?.message || 'Failed to load sent emails.'}
              onRetry={refetch}
            />
          ) : emails.length === 0 ? (
            <EmptyState
              ...
              description="Emails you compose and send will appear here."
            />
          ) : (
            emails.map((email) => (
              <EmailListItem
        ...
        {selectedEmailId ? (
          isThreadLoading ? (
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-slate-950">
              ...
              <p className="text-xs text-slate-500 mt-1">Retrieving sent message details</p>
            </div>
          ) : activeThread ? (
            <ThreadView
              thread={activeThread}
          ...
          <div className="hidden lg:flex flex-col items-center justify-center h-full text-center p-8 select-none">
            ...
            <p className="text-sm text-slate-400 font-medium">Select a sent email to view conversation</p>
          </div>
        )}
      </div>
    </div>
