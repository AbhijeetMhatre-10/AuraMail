...
export function ArchivePage() {
  ...
  const unarchiveMutation = useMutation({
    mutationFn: (id: string) => emailsApi.unarchiveEmail(id),
    onSuccess: (_, unarchivedId) => {
      queryClient.invalidateQueries({ queryKey: ['emails'] });
      queryClient.invalidateQueries({ queryKey: ['thread'] });
      queryClient.invalidateQueries({ queryKey: ['activity'] });
      if (selectedEmailId === unarchivedId) {
        setSelectedEmailId(null);
      }
      success('Email unarchived (moved to All Mail / Inbox)');
    },
    onError: (err: any) => {
      error('Failed to unarchive', err.message);
    },
  });
      ...
      >
        <div className="px-4 py-3 border-b border-slate-800 bg-slate-900/90 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-white">Archived Mail</h2>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-slate-850/80">
          {isLoading ? (
            ...
            <ErrorState
              message={(queryError as any)?.message || 'Failed to load archived emails.'}
              onRetry={refetch}
            />
          ) : emails.length === 0 ? (
            <EmptyState
              ...
              description="Archived emails are preserved here and can be unarchived back to your inbox anytime."
            />
          ) : (
            emails.map((email) => (
              <EmailListItem
                ...
                onArchive={(e) => {
                  e.stopPropagation();
                  unarchiveMutation.mutate(email.id || email._id!);
                }}
                onDelete={(e) => {
                  e.stopPropagation();
      ...
      >
        {selectedEmailId && activeThread ? (
          ...
          <div className="hidden lg:flex flex-col items-center justify-center h-full text-center p-8 select-none">
            ...
            <p className="text-sm text-slate-400 font-medium">Select an archived message to read</p>
          </div>
        )}
      </div>
    </div>
