...
export function AIInsightPanel({ analysis, isLoading }: AIInsightPanelProps) {
  if (isLoading) {
    return (
  ...
  if (!analysis) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-800 bg-slate-900/30 p-6 text-center">
        ...
        <p className="text-sm font-medium text-slate-400">No AI analysis available yet</p>
        <p className="text-xs text-slate-500 mt-1">AI analysis will process message signals automatically.</p>
      </div>
    );
  }
